(function(){
  ...
  ready(function(){
    const cvs=document.getElementById("utilizationChart"); if(!cvs) return;
    const ctx=ctx2d(cvs); if(!ctx) return;
    const D=J(); const F=window.ANALYTICS_DUMMY||{};
    const labels=(Array.isArray(D?.utilization?.labels)&&D.utilization.labels.length?D.utilization.labels:F.utilization.labels);
    const vals=(Array.isArray(D?.utilization?.values)&&D.utilization.values.length?D.utilization.values:F.utilization.values).map(n=>+n||0);
    ...
    ctx.fillStyle="rgba(128,20,60,.07)"; ctx.fill();
    ...
    labels.forEach((lb,i)=>ctx.fillText(lb, X(i)-6, H-12));
