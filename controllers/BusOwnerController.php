...
class BusOwnerController extends BaseController
{
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if ($act === 'create') {
                ...
                $v->required('reg_no', 'Registration Number')
                    ...
                    ->inList('bus_class', 'Bus Class', ['Normal', 'Semi-Luxury', 'AC'])
    ...
    public function drivers()
    {
        ...
        $this->view('bus_owner', 'drivers', [
            ...
            'opId' => $opId,
            'driver_logs' => $m->getAllDriverLogs(),
            'conductor_logs' => $m->getAllConductorLogs(),
            'fieldErrors' => $flash['errors'],
            'oldInput' => $flash['old'],
        ]);
    }
    ...
    public function exportEarnings()
    {
        ...
        <Workbook xmlns="urn:schemas-microsoft-com:office:spreadsheet" xmlns:ss="urn:schemas-microsoft-com:office:spreadsheet"
            ...
            <Styles>
                ...
                <Style ss:ID="header">
                    <Font ss:Bold="1" ss:Color="#FFFFFF" /><Interior ss:Color="#80143C" ss:Pattern="Solid" /><Alignment ss:Horizontal="Center" />
                </Style>
                <Style ss:ID="total">
                    <Font ss:Bold="1" /><Interior ss:Color="#FEF3C7" ss:Pattern="Solid" />
                </Style>
                <Style ss:ID="currency">
                    <NumberFormat ss:Format="#,##0.00" />
                </Style>
                <Style ss:ID="totalCurrency">
                    <Font ss:Bold="1" /><Interior ss:Color="#FEF3C7" ss:Pattern="Solid" /><NumberFormat ss:Format="#,##0.00" />
                </Style>
                <Style ss:ID="date">
    ...
    public function feedback()
    {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if ($act === 'resolve') {
                ...
                if ($id !== '')
                    $m->updateStatus((string) $id, 'Resolved');
                if (trim((string) $note) !== '')
                    $m->sendResponse((string) $id, (string) $note);
            ...
            if ($act === 'close') {
                $id = $_POST['complaint_id'] ?? ($_POST['feedback_ref'] ?? ($_POST['id'] ?? ''));
                if ($id !== '')
                    $m->updateStatus((string) $id, 'Closed');
            ...
            $stmt = $pdo->prepare(
                "SELECT
                 ...
                 JOIN private_buses pb
                     ...
                     AND LOWER(COALESCE(pb.status, '')) = 'active'
