...
function attStatus(array $records, string $attendanceKey): string {
    return $records[$attendanceKey]['status'] ?? 'Absent';
}
function attNote(array $records, string $attendanceKey): string {
    return htmlspecialchars($records[$attendanceKey]['notes'] ?? '');
}
...
.hist-filter { display:flex; gap:10px; align-items:center; flex-wrap:wrap; }
.hist-filter label { font-size:.95rem; color:#6b7280; font-weight:700; }
...
.hist-filter .go-sm:hover { background:#a8274e; transform:translateY(-1px); }
...
.history-table tr:hover td { background:#fffdf6 !important; }
...
<section class="att-hero">
    ...
    <div class="att-hero-right">
        ...
        <?php if ($date !== $today): ?>
        <a href="/O/attendance" class="go-btn" style="background:rgba(255,255,255,.15);color:#fff;border:1.5px solid #f3b944;border-radius:9px;padding:8px 16px;text-decoration:none;font-size:.85rem;font-weight:600;">
            &#8635; Today
        </a>
        <?php endif; ?>
    </div>
...
<!-- Date picker bar -->
<form method="get" action="/O/attendance" class="date-bar">
...
<!-- History Section -->
<section class="history-section" id="attendance-history" style="display:block;visibility:visible;opacity:1;position:relative;z-index:2;">
    <div class="history-head">
        <h3>&#128200; Attendance History</h3>
        <form method="get" action="/O/attendance" class="hist-filter" id="hist-date-form" style="display:flex;gap:8px;align-items:center;flex-wrap:wrap;">
            <input type="hidden" name="date" value="<?= htmlspecialchars($date) ?>">
            <label>From</label>
            <input type="date" name="from" value="<?= htmlspecialchars($histFrom) ?>" max="<?= $today ?>" style="width:auto;">
            <label>To</label>
            <input type="date" name="to"   value="<?= htmlspecialchars($histTo)   ?>" max="<?= $today ?>" style="width:auto;">
            <button type="submit" class="go-sm" style="width:auto;">Filter</button>
        </form>
    </div>
    ...
    <table class="history-table" id="hist-table">
        ...
        <tr class="hist-no-results" data-empty-row="1">
            <td colspan="5">No attendance records found for the selected period.</td>
        </tr>
        <?php endif; ?>
        <?php foreach ($history as $h):
            $pillClass = 'pill-' . strtolower((string)($h['status'] ?? 'Present'));
            $typeClass = 'type-' . strtolower((string)($h['staff_type'] ?? 'Driver'));
        ?>
        <tr data-name="<?= strtolower(htmlspecialchars((string)($h['full_name'] ?? ''))) ?>"
            data-type="<?= strtolower((string)($h['staff_type'] ?? '')) ?>"
            data-status="<?= strtolower(str_replace(' ','_',(string)($h['status'] ?? ''))) ?>">
            <td><?= date('d M Y', strtotime((string)$h['work_date'])) ?></td>
            <td><span class="type-badge <?= $typeClass ?>"><?= htmlspecialchars((string)($h['staff_type'] ?? '—')) ?></span></td>
            <td style="font-weight:600;"><?= htmlspecialchars((string)($h['full_name'] ?? '—')) ?></td>
            <td><span class="status-pill <?= $pillClass ?>"><?= htmlspecialchars(str_replace('_',' ',(string)($h['status'] ?? 'Present'))) ?></span></td>
    ...
    (function () {
        ...
        var staticEmptyRow = tbody.querySelector('tr[data-empty-row="1"]');
        var noResRow = null;
        ...
        function filterRows() {
            ...
            if (staticEmptyRow) {
                staticEmptyRow.style.display = allRows.length === 0 ? '' : 'none';
            }
    ...
    // Hard fallback: keep history section visible even if external CSS collapses it.
    (function () {
        var sec = document.getElementById('attendance-history');
        if (!sec) return;
...
<div class="mark-grid">
    ...
    <!-- Drivers Form -->
    <form method="post" action="/O/attendance">
    ...
    <div class="mark-card">
        ...
        <div class="empty-staff">No drivers found for your depot.</div>
        <?php else: ?>
        ...
        <table class="staff-table">
            ...
            <?php foreach ($drivers as $d):
                ...
                $akey = (string)($d['attendance_key'] ?? ('driver:' . (int)$d['id']));
                $sel = $isSuspended ? 'Absent' : attStatus($records, $akey);
            ...
            <tr class="<?= $rowClass ?>">
                ...
                <td>
                        <select name="attendance[<?= htmlspecialchars($akey) ?>]"
                ...
                <td>
                          <input type="text" name="notes[<?= htmlspecialchars($akey) ?>]"
                           class="note-input"
                              value="<?= $isSuspended ? '' : attNote($records, $akey) ?>"
    ...
    <div class="mark-card">
        ...
        <div class="empty-staff">No conductors found for your depot.</div>
        <?php else: ?>
        <div class="staff-table-wrap">
        <table class="staff-table">
            <thead>
            ...
            <?php foreach ($conductors as $c):
                ...
                $akey = (string)($c['attendance_key'] ?? ('conductor:' . (int)$c['id']));
                $sel = $isSuspended ? 'Absent' : attStatus($records, $akey);
                $cls = 'is-' . strtolower(str_replace('_','-',$sel));
                $rowClass = $isSuspended ? 'att-row--suspended' : '';
            ?>
