...
$displayName = $displayName !== '' ? $displayName : ($me['name'] ?? 'Depot Officer');
    ...
    <div class="dop-hero-identity">
      ...
      <div class="dop-hero-meta">
        <span class="dop-hero-chip dop-chip-role">
          ...
          <?= htmlspecialchars($me['role'] ?? 'Depot Officer') ?>
        </span>
  ...
  <section class="dop-card">
    ...
    <form method="post" class="dop-form" id="dop-profile-form">
      ...
      <div class="dop-form-footer">
        ...
        <a href="/O/dashboard" class="dop-btn dop-btn-ghost">
    ...
    <form method="post" class="dop-form" id="dop-pw-form">
      ...
      <div class="dop-field">
        ...
        <div class="dop-input-wrap">
          ...
          <button type="button" class="dop-pw-toggle" data-target="dop-cur-pw" title="Show/hide password" aria-label="Toggle password visibility">
      ...
      <div class="dop-field">
        ...
        <div class="dop-input-wrap">
          ...
          <button type="button" class="dop-pw-toggle" data-target="dop-new-pw" title="Show/hide password" aria-label="Toggle password visibility">
            <svg width="15" height="15" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24" class="dop-eye-icon"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/></svg>
          </button>
        </div>
        <!-- Password strength meter -->
        <div class="dop-strength-wrap" id="dop-strength-wrap" aria-live="polite">
          <div class="dop-strength-bar">
            <div class="dop-strength-fill" id="dop-strength-fill"></div>
          </div>
      ...
      <div class="dop-field">
        ...
        <div class="dop-input-wrap">
          ...
          <button type="button" class="dop-pw-toggle" data-target="dop-conf-pw" title="Show/hide password" aria-label="Toggle password visibility">
            <svg width="15" height="15" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24" class="dop-eye-icon"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/></svg>
          </button>
        </div>
...
.dop-toast {
  ...
  pointer-events: auto;
  animation: dop-toast-in .3s cubic-bezier(.34,1.56,.64,1) both;
  border: 1px solid;
}
.dop-toast.dop-toast-success {
  background: #f0fdf4;
  color: #15803d;
  border-color: #bbf7d0;
}
.dop-toast.dop-toast-error {
  background: #fef2f2;
  color: #b91c1c;
  border-color: #fecaca;
}
.dop-toast.dop-toast-info {
  background: #eff6ff;
  color: #1d4ed8;
  border-color: #bfdbfe;
}
...
@keyframes dop-toast-out {
  from { opacity: 1; transform: translateX(0) scale(1); }
  to   { opacity: 0; transform: translateX(40px) scale(.8); }
}
...
.dop-hero-cover {
  position: absolute;
  inset: 0;
  background: linear-gradient(135deg, #63082e 0%, #80143c 40%, #a41b4e 70%, #c02060 100%);
  z-index: 0;
}
.dop-hero-cover::after {
  content: '';
  position: absolute;
  inset: 0;
  background:
    radial-gradient(ellipse 60% 80% at 80% 50%, rgba(255,255,255,.07) 0%, transparent 70%),
    url("data:image/svg+xml,%3Csvg width='40' height='40' viewBox='0 0 40 40' xmlns='http://www.w3.org/2000/svg'%3E%3Cg fill='none' fill-rule='evenodd'%3E%3Ccircle fill='%23ffffff' opacity='0.04' cx='20' cy='20' r='2'/%3E%3C/g%3E%3C/svg%3E");
}
.dop-hero-body {
  position: relative;
  z-index: 1;
...
.dop-avatar-btn:hover .dop-avatar-overlay,
.dop-avatar-btn:focus-visible .dop-avatar-overlay {
  opacity: 1;
}
...
.dop-input:focus {
  border-color: #80143c;
  background: #fff;
  box-shadow: 0 0 0 3px rgba(128,20,60,.12);
}
...
.dop-btn-primary {
  ...
  color: #fff;
  border-color: #80143c;
  box-shadow: 0 2px 8px rgba(128,20,60,.25);
}
...
(function() {
  ...
  function showToast(text, type) {
    var icons = {
      ...
      error:   '<svg width="18" height="18" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg>',
    ...
    t.innerHTML = '<span class="dop-toast-icon">' + (icons[type] || icons.info) + '</span>'
                + '<span class="dop-toast-text">' + text + '</span>'
                + '<button class="dop-toast-close" aria-label="Dismiss">&times;</button>';
    ...
    function dismiss(el) {
      ...
      el.style.animation = 'dop-toast-out .25s ease forwards';
      el.addEventListener('animationend', function() { el.remove(); });
    }
  }

  /* ── Fire page-load flash from PHP ─────────────────────────────── */
  <?php if ($_flashMsg): ?>
  document.addEventListener('DOMContentLoaded', function() {
    showToast(<?= json_encode($_flashMsg, JSON_HEX_TAG | JSON_HEX_AMP | JSON_HEX_QUOT) ?>, <?= json_encode($_flashType) ?>);
  });
  <?php endif; ?>
  ...
  document.addEventListener('DOMContentLoaded', function() {
    ...
    form.addEventListener('submit', async function(e) {
      ...
      var res, data;
      try {
        res = await fetch(location.pathname, {
          method: 'POST',
      ...
      if (data && data.ok) {
        ...
        var nameEl = document.getElementById('dop-display-name');
        if (nameEl) nameEl.textContent = dn;
        var emailEl = document.getElementById('dop-display-email');
        if (emailEl) emailEl.childNodes[emailEl.childNodes.length - 1].textContent = u.email || '';
        showToast('Profile updated successfully.', 'success');
      } else {
        showToast((data && data.msg) || 'Could not update profile.', 'error');
      }
    });
  });
  ...
  document.addEventListener('DOMContentLoaded', function() {
    ...
    if (inp) inp.addEventListener('change', function() {
      if (this.files.length > 0) {
        /* Preview immediately before submit so user sees change right away */
        var reader = new FileReader();
        reader.onload = function(ev) {
          var btn = document.querySelector('.dop-avatar-btn');
          if (btn) {
            btn.innerHTML = '<img src="' + ev.target.result + '" alt="Profile photo" class="dop-avatar-img" id="dop-avatar-img">'
              + btn.querySelector('.dop-avatar-overlay').outerHTML;
          }
        };
  ...
  document.addEventListener('DOMContentLoaded', function() {
    ...
    delBtn.addEventListener('click', async function() {
      ...
      if (avatarBtn) {
        ...
        avatarBtn.innerHTML = '<span class="dop-avatar-initial">' + initial + '</span>'
          + '<span class="dop-avatar-overlay" aria-hidden="true">'
          + '<svg width="22" height="22" fill="none" stroke="#fff" stroke-width="2" viewBox="0 0 24 24"><path d="M23 19a2 2 0 0 1-2 2H3a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h4l2-3h6l2 3h4a2 2 0 0 1 2 2z"/><circle cx="12" cy="13" r="4"/></svg>'
          + '<span>Change Photo</span></span>';
      }
      delBtn.style.display = 'none';
      delBtn.removeAttribute('data-loading');
  ...
  document.addEventListener('DOMContentLoaded', function() {
    ...
    var levels = [
      ...
      { pct:'60%', color:'#eab308', text:'Moderate'  },
  ...
  document.addEventListener('DOMContentLoaded', function() {
    ...
    function check() {
      ...
      if (confPw.value === newPw.value) {
        hint.textContent = '✓ Passwords match';
        hint.className = 'dop-match-hint ok';
      } else {
        hint.textContent = '✗ Passwords do not match';
        hint.className = 'dop-match-hint no';
      }
    }
