<?php
/* vars: timetable_rows, count_total, depot_name, msg */
 ...
 */
function groupTimetableRows(array $rows, array $dayMap): array {
    $groups = [];
    foreach ($rows as $r) {
...
$allGroups = groupTimetableRows($timetableRows, $dayMap);
...
<section class="tt2-hero">
    <div>
        ...
        <p>Read-only schedule viewer for all depot schedules</p>
    </div>
...
<div class="tt2-notice">
    &#128274;&ensp;This page is read-only for Depot Officers. Schedules are managed by NTC Admin and Depot Manager.
</div>
<?php endif; ?>
...
<div class="tt2-filters" id="tt2Filters">
    ...
    <div class="tt2-filter-section">
        ...
        <span style="font-size:.75rem;color:#9ca3af;font-style:italic;">Schedules are filtered by their effective date window when available.</span>
    </div>
...
<div class="tt2-card">
    ...
    <div class="tt2-readonly-bar">
        ...
        Read-only schedules managed by NTC Admin and Depot Manager.
    </div>
    ...
    <table class="tt2-table" id="tt2Table">
        ...
        ?>
        <tr data-days="<?= htmlspecialchars($daysAttr) ?>"
        ...
        <tr class="tt2-no-results tt2-hidden" id="tt2NoResults">
            <td colspan="6">No schedules match the selected filters.</td>
        </tr>
        </tbody>
    </table>
    </div>
...
(function () {
    ...
    function applyFilters() {
        ...
        rows.forEach(row => {
            ...
            const rEffFrom = row.dataset.effFrom;
            const rEffTo   = row.dataset.effTo;
            ...
            // 2. Day filter (any selected day must be in row's days)
            if (selDays.length > 0 && !selDays.some(d => rDays.includes(d))) {
                hide(row); return;
            }

            // 3. Period filter (overlap check when effective dates exist)
            if (fromVal || toVal) {
