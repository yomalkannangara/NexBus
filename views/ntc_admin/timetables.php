    ...
    <label>Route
      <select name="route_id" required>
        <?php foreach($routes as $r): ?>
          <option value="<?= htmlspecialchars($r['route_id']) ?>"><?= htmlspecialchars($r['route_no']) ?></option>
        <?php endforeach; ?>
      </select>
    </label>
...
<section class="table-panel">
  <div class="table-panel-head">
    <h2>Bus Schedules</h2><div>
    <button class="btn primary" id="showAddTT">+ Add Schedule</button>  
    <button class="btn primary" id="showAddRoute">+ Add Route</button>   </div>
  </div>
