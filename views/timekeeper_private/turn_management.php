...
<div class="card">
  <div class="table-wrap">
    <table class="tk-table" id="turnTable">
      ...
      <tbody>
      <?php if (empty($rows ?? [])): ?>
        <tr><td colspan="6" style="text-align:center;padding:28px;color:#9ca3af;">No trips currently running at this stop.</td></tr>
      <?php else: ?>
      <?php foreach (($rows ?? []) as $r): $delay = (int)($r['delay_min'] ?? 0); $tripStatus = $r['trip_status'] ?? 'InProgress'; ?>
        <tr class="row" data-trip-id="<?= (int)$r['private_trip_id'] ?>">
          ...
          <td data-label="Action">
            <button class="btn btn-complete" data-action="complete" data-trip-id="<?= (int)$r['private_trip_id'] ?>">&#10003; Close</button>
            <button class="btn btn-cancel"   data-action="cancel"   data-trip-id="<?= (int)$r['private_trip_id'] ?>">&#215; Stop Trip</button>
          </td>
        </tr>
      <?php endforeach; ?>
      <?php endif; ?>
...
(function () {
  ...
  document.getElementById('turnTable').addEventListener('click', function (e) {
    ...
    if (action === 'complete') {
      if (!confirm('Mark this trip as Completed (arrived at end stop)?')) return;
      btn.disabled = true;
  ...
  confirmBtn.addEventListener('click', function () {
    ...
    postAction({ action: 'cancel', trip_id: id, reason: reason }, function () {
      overlay.style.display = 'none';
      pendingTripId = 0;
      showToast('Trip stopped and operator notified.', 'success');
      setTimeout(function () { location.reload(); }, 900);
    }, confirmBtn);
  });
})();
