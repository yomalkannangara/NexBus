...
<div class="grid" style="display:grid;grid-template-columns:repeat(3,minmax(220px,1fr));gap:16px;margin:16px 0;">
  <div class="card accent-rose">
    <div class="metric-title">Total Buses in Your Depot</div>
    <div class="metric-value"><?= (int)($stats['total_buses'] ?? 0) ?></div>
    <div class="metric-sub">All buses linked to your depot</div>
  </div>

  <div class="card accent-blue">
    <div class="metric-title">Total Trips Today</div>
    <div class="metric-value"><?= (int)($stats['total_trips_today'] ?? 0) ?></div>
    <div class="metric-sub">All scheduled depot trips for today</div>
  </div>
...
<section class="filters tk-map-filters">
  ...
  <div class="filter-grid">
    <div>
      <label for="tk-sltb-map-filter-route">Route</label>
      <select id="tk-sltb-map-filter-route" onchange="applySltbMapFilters()">
        <option value="">All Routes</option>
      </select>
    </div>
    <div>
      <label for="tk-sltb-map-filter-bus">Bus Number</label>
      <select id="tk-sltb-map-filter-bus" onchange="applySltbMapFilters()">
  ...
  <div class="tk-map-live-stats">
    ...
    <span class="lf-badge lf-badge--green" id="tk-sltb-live-count">- buses</span>
    <span class="lf-badge lf-badge--red" id="tk-sltb-speed-viols">- speeding</span>
    <span class="tk-map-updated" id="tk-sltb-map-updated"></span>
  </div>
</section>
...
<div class="card tk-map-card">
  ...
  <div id="tk-sltb-live-map" class="tk-live-map"></div>

  <script src="https://unpkg.com/leaflet@1.9.4/dist/leaflet.js" crossorigin=""></script>
  <script>
  (function(){
    var mapEl = document.getElementById('tk-sltb-live-map');
    var routeSelect = document.getElementById('tk-sltb-map-filter-route');
    var busSelect = document.getElementById('tk-sltb-map-filter-bus');
    if (!mapEl || !routeSelect || !busSelect || typeof L === 'undefined') return;

    var map = L.map('tk-sltb-live-map').setView([6.927, 79.861], 12);
    ...
    window.applySltbMapFilters = function(){
    ...
    function renderBuses(buses){
      ...
      var c = document.getElementById('tk-sltb-live-count');
      var v = document.getElementById('tk-sltb-speed-viols');
      var u = document.getElementById('tk-sltb-map-updated');
