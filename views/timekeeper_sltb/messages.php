...
function ts_msg_time_ago(?string $ts): string
{
    if (!$ts) {
        return 'Unknown time';
    }
...
function ts_msg_type_class(string $type): string
{
    return match ($type) {
...
function ts_msg_source(array $row): string
{
    $name = trim((string)($row['source_name'] ?? ''));
    return $name !== '' ? $name : 'Depot Messaging';
}

function ts_msg_priority(array $row): ?string
{
...
<div class="tmsg-page">
    <div class="tmsg-hero">
        <div>
            <h1>SLTB Timekeeper Messages</h1>
            <p><?= htmlspecialchars($S['depot_name'] ?? 'Depot') ?> notifications from depot operations.</p>
        </div>
    ...
    <div class="tmsg-list">
        ...
        <?php else: ?>
            <?php foreach ($recent as $row): ?>
                ...
                $priority = ts_msg_priority($row);
                ?>
                <article class="tmsg-item <?= $isUnread ? 'unread' : '' ?>">
                    <div class="tmsg-top">
                        <div>
                            <h3 class="tmsg-title"><?= htmlspecialchars(ts_msg_source($row)) ?></h3>
                            <div class="tmsg-meta">
                                <?= htmlspecialchars($sourceRole !== '' ? $sourceRole : 'Depot Officer') ?>
                                | <?= htmlspecialchars(ts_msg_time_ago((string)($row['created_at'] ?? ''))) ?>
                            </div>
                        </div>
                        <div class="tmsg-tags">
                            <span class="tmsg-tag <?= ts_msg_type_class($type) ?>"><?= htmlspecialchars($type) ?></span>
                    ...
                    <div class="tmsg-actions">
                        <?php if ($isUnread && $id > 0): ?>
                            <form method="post" action="/TS/messages?action=read&amp;id=<?= $id ?>&amp;filter=<?= urlencode($filter) ?>">
