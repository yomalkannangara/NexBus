...
class DepotOfficerModel extends BaseModel
{
    ...
    public function sendMessage(
        ...
        return $this->msg->send($depotId, $userIds, $text, $priority, $scope, $allDepot);
    }

    public function recentMessages(int $depotId, int $myId, int $limit = 50, string $filter = 'all'): array {
        return $this->msg->recent($depotId, $limit, $filter);
    }

    public function markMessageRead(int $notificationId, int $userId): void {
        $this->msg->markRead($notificationId, $userId);
    }
